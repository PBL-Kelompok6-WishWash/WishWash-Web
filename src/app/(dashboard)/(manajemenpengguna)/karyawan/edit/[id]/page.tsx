...
export default function EditKaryawanPage() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const result = await karyawanService.getById(id);
        ...
        setFormData({
          ...
          status_ketersediaan: data.status_ketersediaan || "Tersedia"
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sessionStorage.setItem('karyawan_notif', `Berhasil memperbarui karyawan ${formData.nama_karyawan} (ID: ${id})!`);
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="bg-[#1e3a5f] p-5 text-white flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h3 className="text-base font-bold leading-tight">Formulir Edit Karyawan</h3>
