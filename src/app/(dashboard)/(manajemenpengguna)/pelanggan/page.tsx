...
export default function PelangganPage() {
  ...
  useEffect(() => {
    const savedNotif = sessionStorage.getItem('pelanggan_notif');
    if (savedNotif) {
      setNotif({ message: savedNotif, type: 'success' });
      sessionStorage.removeItem('pelanggan_notif'); 
    }
  }, [pathname]);
  ...
  const fetchPelanggan = async () => {
    setIsLoading(true);
    try {
      const result = await pelangganService.getAll();
      setData(result.data || []);
    } catch (error) {
  ...
  const handleDelete = async (id: number, nama: string) => {
    if (!window.confirm(`Yakin ingin menghapus pelanggan ${nama}? Akun user-nya juga ikut terhapus.`)) return;

    try {
      await pelangganService.delete(id);
      setData(data.filter(item => item.id_pelanggan !== id));
      
      setNotif({ message: `Berhasil menghapus pelanggan ${nama} (ID: ${id})!`, type: 'success' });
      
    } catch (error) {
      console.error(error);
      setNotif({ message: `Gagal menghapus pelanggan ${nama}.`, type: 'error' });
    }
  };
  ...
  //  FUNGSI BANTUAN: Render Icon Sort yang selalu nampil 2 panah
          ...
          <table className="w-full text-left border-collapse border-y border-slate-200 min-w-[800px]">
            ...
            <tbody className="text-sm">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={7} className="p-10 text-center text-slate-400 font-medium border-b border-slate-200">
                    {searchQuery ? "Data tidak ditemukan." : "Belum ada data pelanggan."}
                  </td>
                </tr>
              ) : (
