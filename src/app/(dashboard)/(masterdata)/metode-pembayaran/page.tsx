                ...
                paginatedData.map((row) => (
                  <tr key={row.id_metode_pembayaran} className="border-b border-slate-200 hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-2 px-3 border-x border-slate-200 font-medium">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] uppercase font-black border ${
                        row.tipe_metode === 'Midtrans' ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-slate-50 text-slate-600 border-slate-200'
                      }`}>
                        {row.tipe_metode}
                      </span>
                    </td>
