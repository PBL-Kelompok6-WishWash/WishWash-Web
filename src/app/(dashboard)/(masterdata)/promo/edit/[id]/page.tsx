...
export default function EditPromoPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await promoService.getById(id);
        const d = result.data;
        const isPersentase = d.tipe_promo === 'Persentase';
        const rawNominal = String(Math.round(d.nominal_potongan ?? 0));

        setFormData({
          kode_promo: d.kode_promo || '',
          nama_promo: d.nama_promo || '',
          deskripsi: d.deskripsi || '',
          tipe_promo: d.tipe_promo || 'Persentase',
          minimal_order: formatThousands(String(Math.round(d.minimal_order ?? 0))),
          maksimal_potongan: formatThousands(String(Math.round(d.maksimal_potongan ?? 0))),
          tgl_mulai: d.tgl_mulai ? d.tgl_mulai.slice(0, 10) : '',
          tgl_berakhir: d.tgl_berakhir ? d.tgl_berakhir.slice(0, 10) : '',
          status_promo: d.status_promo || 'Aktif',
          gambar_promo: d.gambar_promo || '',
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sessionStorage.setItem('promo_notif', `Berhasil memperbarui promo ${formData.nama_promo} (ID: ${id})!`);
      router.push('/promo');
    } catch (err: any) {
      setErrorMsg(err.error || 'Gagal mengupdate promo');
      setIsLoading(false);
    }
  };
  ...
  if (isFetching) {
    return (
      <div className="w-full h-[60vh] flex flex-col items-center justify-center">
        <div className="w-10 h-10 border-4 border-slate-200 border-t-[#4FD1D9] rounded-full animate-spin mb-4" />
  ...
  return (
    <div className="w-full max-w-4xl mx-auto relative pb-10">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-3xl font-black text-[#1e3a5f] tracking-wider uppercase">Edit Promo</h2>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
        <div className="bg-[#1e3a5f] p-5 text-white flex items-center gap-4">
          ...
          <div>
            <h3 className="text-base font-bold leading-tight">Formulir Edit Promo</h3>
            <p className="text-[11px] text-slate-300 font-medium mt-0.5">Ubah detail kode promo dan periode berlakunya.</p>
          </div>
        </div>
