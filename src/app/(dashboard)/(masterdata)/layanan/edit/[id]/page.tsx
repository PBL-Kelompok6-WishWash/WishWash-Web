      ...
      try {
        ...
        if (data.paket_layanan && data.paket_layanan.length > 0) {
           setPakets(data.paket_layanan.map((p: any, i: number) => ({
             ...
             biaya_tambahan: p.biaya_tambahan.toString()
           })));
        }
      } catch (error) {
        ...
        <div className="bg-[#1e3a5f] p-5 text-white flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[11px] text-slate-300 font-medium leading-tight mt-0.5">
               Perbarui jenis layanan, urutan status pesanan, beserta variasi paket.
            </p>
          </div>
        </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    className={`w-full px-4 py-3 rounded-xl border-2 font-medium flex items-center justify-between transition-all ${
                      isUsed 
                        ? 'border-slate-100 bg-slate-50 text-slate-400 cursor-not-allowed' 
                        : 'border-slate-100 bg-white text-[#1e3a5f] hover:border-[#4FD1D9]/60 focus:border-[#4FD1D9] focus:outline-none'
                    }`}
                  >
                    <span>{formData.jenis_satuan}</span>
                    <ChevronDown size={18} className={`transition-transform duration-300 ${isJenisSatuanOpen ? 'rotate-180' : 'rotate-0'} ${isUsed ? 'text-slate-300' : 'text-[#1e3a5f]'}`} />
                  </button>
